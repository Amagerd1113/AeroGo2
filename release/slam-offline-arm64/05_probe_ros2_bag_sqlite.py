...
def main():
    if len(sys.argv) != 2:
        raise SystemExit("usage: 05_probe_ros2_bag_sqlite.py /absolute/path/to/ros2_bag")
    bag_path = pathlib.Path(sys.argv[1]).expanduser().resolve()
    database_paths = sorted(bag_path.glob("*.db3"))
    if not (bag_path / "metadata.yaml").is_file() or not database_paths:
        raise SystemExit(f"ERROR: ROS 2 sqlite3 bag not found under {bag_path}")
    ...
    for database_path in database_paths:
        ...
        try:
            query = """
                SELECT messages.timestamp, messages.data, topics.name
                FROM messages
                JOIN topics ON messages.topic_id = topics.id
                WHERE topics.name IN ('/utlidar/imu', '/utlidar/cloud')
                ORDER BY messages.timestamp
            """
            for record_time_ns, payload, topic in connection.execute(query):
                if topic == "/utlidar/imu":
                    message = deserialize_message(bytes(payload), Imu)
                    ...
                    message = deserialize_message(bytes(payload), PointCloud2)
                    header_time = stamp_seconds(message)
                    ...
                    if not required.issubset(field_offsets):
                        ...
                        raise SystemExit(f"ERROR: missing PointCloud2 fields: {missing}")
                    ...
                    for index in range(point_count):
                        ...
                        point_time = struct.unpack_from(
                            endian + "f", message.data, base + field_offsets["time"]
                        )[0]
                        if math.isfinite(point_time):
                            point_times.append(point_time)
                        if index % point_stride == 0:
                            x = struct.unpack_from(
                                endian + "f", message.data, base + field_offsets["x"]
                            )[0]
                            y = struct.unpack_from(
                                endian + "f", message.data, base + field_offsets["y"]
                            )[0]
                            z = struct.unpack_from(
                                endian + "f", message.data, base + field_offsets["z"]
                            )[0]
    ...
    imu_header_reversals = reversals([row[0] for row in imu_rows])
    cloud_header_reversals = reversals(cloud_stamps)
    ...
    axis_stats = [mean_std([row[index] for row in static_rows]) for index in range(1, 7)]
    acceleration_norms = [
        math.sqrt(row[1] ** 2 + row[2] ** 2 + row[3] ** 2) for row in static_rows
    ]
    angular_norms = [
        math.sqrt(row[4] ** 2 + row[5] ** 2 + row[6] ** 2) for row in static_rows
    ]
    ...
    print(f"imu_nonmonotonic={imu_header_reversals}")
    print(f"cloud_nonmonotonic={cloud_header_reversals}")
