...
本文描述当前代码中的顶层 `SystemState`，不是 ArduPilot 的 `STABILIZE`、`LOITER`、`RTL` 等飞行模式。AeroGo2 只读取 Pixhawk 飞行模式；飞行模式切换仍由 RadioMaster/ArduPilot 负责。当前完整节点图见 [AEROGO2_CURRENT_PROJECT_GRAPH_ZH.svg](AEROGO2_CURRENT_PROJECT_GRAPH_ZH.svg)，Impact-aware 算法、参数和硬件门禁见 [IMPACT_AWARE_MPC_INTEGRATION_ZH.md](IMPACT_AWARE_MPC_INTEGRATION_ZH.md)。
...
| `FLIGHT_MANUAL` | `FLIGHT_READY`, `AUTO_LANDING_READY`, `TOUCHDOWN_VERIFY`, `FLIGHT_TO_WALK_PRECHECK`, `FAULT`, `EMERGENCY_STOP` | 从 `FLIGHT_READY` 进入时必须同时观察到未过期的一次性 Shell 授权、有效 Go2 控制权和 Pixhawk armed。授权进入后立即消费。进入后必须由新鲜 Pixhawk 遥测连续证明 `armed=true` 且 `landed=false` 达 `safety.airborne_confirm_s`，才锁存本架次的 `AIRBORNE_CONFIRMED` 并启用触地检测。锁存前若收到新鲜的 `armed: true -> false` 边沿且 `landed=true`，自动回 `FLIGHT_READY`，必须把 CH5 拉回 LOW 并重新 `flight authorize`；锁存离地后禁用这条回退，避免空中 Disarm 被当成地面重试。地面等待不会直接进入 `TOUCHDOWN_VERIFY`。高层权威时检查 JOINT_LOCK；LowCmd 权威时检查 owner、LowState 与跟踪误差；两种状态不得混用。`AUTO_LANDING_READY` 在双硬件写入开关开启、向下测距/速度新鲜时也可进入；MPC 必须在这个飞行状态内通过 `autoland prepare mpc` 和精确短语 `CONFIRM_MPC_AUTOLAND` 逐次选择，配置开关本身不会选择。 |
| `AUTO_LANDING_READY` | `AUTO_LANDING`, `FLIGHT_MANUAL`, `FAULT`, `EMERGENCY_STOP` | DRY-RUN 可用；真机还要求 HW 写入双开关、向下 DISTANCE_SENSOR/速度估计有效。要求 Pixhawk armed、CH10 为 AUTO_READY/AUTO_EXECUTE、落地估计有效。普通 `autoland prepare` 锁存 legacy；只有能力开关已开且飞行中确认过 `autoland prepare mpc` 才锁存本次 MPC。任何人工接管/中止返回 `FLIGHT_MANUAL`，停止外部 setpoint 并清除本次选择。CH10 的 MANUAL/LOW 退出信号直接读取当前原始 RC 帧，不等待进入自动档所用的防抖。 |
| `AUTO_LANDING` | `TOUCHDOWN_VERIFY`, `FLIGHT_MANUAL`, `FAULT`, `EMERGENCY_STOP` | 普通 SafeDescent 已开放到受守卫的 HW 路径；开始前确认 GUIDED、每条指令前复查模式和心跳、接管时确认人工模式。LowCmd-enabled 路径仍会返回 `COORDINATED_ACTUATION_NOT_CONFIGURED`。普通准备或未确认 MPC 时保持 legacy SafeDescent 路径，要求 CH10=AUTO_EXECUTE、Pixhawk armed 且无 failsafe、RC 新鲜无人工接管、落地估计有效且所有数值有限。CH10 原始 MANUAL/LOW 或已标定 CH1–CH4 任一摇杆超过 deadband 会立即中止自动输出、保留 armed 并返回 `FLIGHT_MANUAL`。只有本次飞行已显式锁存 `autoland_mpc_selected` 时才启用 Impact-aware 状态流程：检测触地后仍停留在本状态，继续完成 `POST_TOUCHDOWN_RECOVERY`；恢复证据、飞控 residual CLEAR/执行回读/持续零状态、Go2 safe-hold 和稳定驻留全部通过后才允许进入 `TOUCHDOWN_VERIFY`。能力开关和运行时选择目前都不接通真实 MPC/LowCmd/飞控输出。 |
| `TOUCHDOWN_VERIFY` | `MANUAL_POSITIONING`, `FLIGHT_MANUAL`, `LANDING_COMPLIANT`, `GO2_GROUND_HANDOVER`, `FLIGHT_TO_WALK_PRECHECK`, `FAULT`, `EMERGENCY_STOP` | 只有本架次已经锁存 `AIRBORNE_CONFIRMED` 才会自动触发。legacy 路径要求 Pixhawk `landed=true`、速度/姿态/ESC/高度变化满足阈值并连续驻留；本次飞行已确认并锁存的 Impact-aware 路径在进入前已经完成更严格的接触后退出屏障，不能把“首次检测到触地”当成进入条件。LowCmd 仍可能保持 conservative hold；需要交还高层控制权时先进入 `GO2_GROUND_HANDOVER`，不得直接启动 F446 或假定 mode=6 已恢复。 |
...
- 启用后，`LANDING_COMPLIANT` 只处理传统高层落地柔顺；`landing.mpc_enabled=true` 且本次飞行已执行确认命令所选择的 Impact-aware 路径则在 `AUTO_LANDING` 内保持 `POST_TOUCHDOWN_RECOVERY`，直到 residual 清零 ACK/执行回读/持续状态、Go2 safe-hold 和稳定驻留全部满足后才进入 `TOUCHDOWN_VERIFY`。两条路径不可用同一个“柔顺完成”标志互相替代，MPC 开关也不代替 LowCmd 硬件许可。
...
- 普通 `SafeDescentController` 自动降落可在显式 HW 写入双开关下运行；要求向下测距和速度新鲜、GUIDED 回读确认，并在每条 setpoint 前复查。缺少任一条件立即拒绝或退出。
- 自动降落开始时 AeroGo2 请求 GUIDED；接管时若飞手已切换模式则保留该模式，否则确认 LOITER 后停止外部 setpoint。RadioMaster 始终保留更高优先级的接管请求。
- MPC/LowCmd/residual 真机链仍阻断，`autoland hardware` 会列出缺失的生产执行端和确认链。
