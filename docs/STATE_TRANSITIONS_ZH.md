...
| `AUTO_LANDING_READY` | `AUTO_LANDING`, `FLIGHT_MANUAL`, `FAULT`, `EMERGENCY_STOP` | 当前只允许 DRY-RUN。要求 Pixhawk armed、CH10 为 AUTO_READY/AUTO_EXECUTE、落地估计有效。普通 `autoland prepare` 锁存 legacy；只有能力开关已开且飞行中确认过 `autoland prepare mpc` 才锁存本次 MPC。任何人工接管/中止返回 `FLIGHT_MANUAL`，停止外部 setpoint 并清除本次选择。 |
| `AUTO_LANDING` | `TOUCHDOWN_VERIFY`, `FLIGHT_MANUAL`, `FAULT`, `EMERGENCY_STOP` | 当前只允许 DRY-RUN；并且 LowCmd-enabled 路径会进一步返回 `COORDINATED_ACTUATION_NOT_CONFIGURED`。普通准备或未确认 MPC 时保持 legacy SafeDescent 路径，要求 CH10=AUTO_EXECUTE、Pixhawk armed 且无 failsafe、RC 新鲜无人工接管、落地估计有效且所有数值有限。只有本次飞行已显式锁存 `autoland_mpc_selected` 时才启用 Impact-aware 状态流程：检测触地后仍停留在本状态，继续完成 `POST_TOUCHDOWN_RECOVERY`；恢复证据、飞控 residual CLEAR/执行回读/持续零状态、Go2 safe-hold 和稳定驻留全部通过后才允许进入 `TOUCHDOWN_VERIFY`。能力开关和运行时选择目前都不接通真实 MPC/LowCmd/飞控输出。 |
